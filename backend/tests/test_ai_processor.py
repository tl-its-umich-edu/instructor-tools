...
class TestAltTextProcessor(TestCase):
    @patch('backend.canvas_app_explorer.alt_text_helper.ai_processor.OpenAI')
    def test_generate_alt_text_raises_alt_text_generation_exception_on_client_error(self, mock_openai):
        """generate_alt_text should raise AltTextGenerationException when the AI client fails."""
        mock_client = MagicMock()
        mock_openai.return_value = mock_client
        mock_client.chat.completions.with_raw_response.create.side_effect = RuntimeError('gateway request failed')
        ...
        self.assertIn('gateway request failed', str(ctx.exception.cause))

    @patch('backend.canvas_app_explorer.alt_text_helper.ai_processor.OpenAI')
    def test_generate_alt_text_raises_alt_text_generation_exception_on_empty_choices(self, mock_openai):
